 ...
 * Bulk create locations (up to 100 per request)
 */
import { NextRequest, NextResponse } from 'next/server'
import { getClient } from '@/lib/db'
import { bulkInsert } from '@/lib/bulk/bulkInsert'
import { CreateManyLocationsSchema, LOCATION_COLUMNS } from '@/lib/schemas/location'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = CreateManyLocationsSchema.parse(body)

    await client.query('BEGIN')
    const locations = await bulkInsert<Location>(
      'locations',
      LOCATION_COLUMNS as unknown as string[],
      validated,
      client
    )
    ...
    return successResponse(
      { created: locations.length, items: locations },
      `Successfully created ${locations.length} ${locations.length === 1 ? 'location' : 'locations'}`,
      201
    )
  } catch (error) {
    ...
    return errorResponse(`Failed to create locations: ${error}`, 500)
  } finally {
    client.release()
  }
}
