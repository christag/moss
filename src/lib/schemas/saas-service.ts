...
export const CreateSaaSServiceSchema = z.object({
  ...
  service_url: z.string().max(255).optional(),
  ...
  seat_count: z.number().int().min(0).optional(),
  ...
  sso_protocol: z.string().max(50).optional(),
  scim_enabled: z.boolean().default(false),
  provisioning_type: z.string().max(50).optional(),
  api_access_enabled: z.boolean().default(false),
  api_documentation_url: z.string().max(255).optional(),
